...
public class CrptApi {
    ...
    public CrptApi(TimeUnit timeUnit, int requestLimit) {
        this.tokenBucket = new TokenBucket(requestLimit, timeUnit);// тут реализована работа с многопоточным доступом и с получением возможзности на выполнение запроса, Если делать не только с либами мапперов, можно было бы реализовать через resilence4j
    ...
    private class CrptAdapter {
        ...
        public void createDocument(Document document, String signature) throws IOException, InterruptedException, NoSuchAlgorithmException, KeyManagementException {
            ...
            // Работа с сертами не реализована
            ...
            System.out.printf("Response message: \n%s%n", response.body()); // поскольку не было указаний что делать с ответом - просто выводим его
        }
